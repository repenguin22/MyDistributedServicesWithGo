...
import (
	"context"
	api "github.com/repenguin22/proglog/api/v1"
	"github.com/repenguin22/proglog/internal/config"
	"github.com/repenguin22/proglog/internal/log"
...
func setupTest(t *testing.T, fn func(*Config)) (client api.LogClient, cfg *Config, teardown func()) {
	...
	clientTLSConfig, err := config.SetupTLSConfig(config.TLSConfig{CAFile: config.CAFile, KeyFile: config.ClientKeyFile, CertFile: config.ClientCertFile})
	require.NoError(t, err)
...
func testConsumePastBoundary(t *testing.T, client api.LogClient, config *Config) {
	...
	got := status.Code(err)
	want := status.Code(api.ErrOffsetOutRange{}.GRPCStatus().Err())
	if got != want {
